...
class Conversation(object):
    ...
    def save_text(self, slot, uid):  # 최종 slot 텍스트로 저장
        ...
        for i in range(1, (len(QA_list_Before.before_slot_list) - 1)):

            if i < len(QA_list_Before.before_slot_list) - 2:
                if QA_list_Before.before_slot_list[i] in slot:
                    file.write(str(QA_list_Before.before_slot_list[i]) + ':' + str(slot[QA_list_Before.before_slot_list[i]]) + "\n")
                    # print(QA_list_Before.before_slot_list[i])
                    # print(slot[QA_list_Before.before_slot_list[i]])
            else:
                if QA_list_Before.before_slot_list[i] in slot:
                    file.write(str(QA_list_Before.before_slot_list[i]) + ':' + str(slot[QA_list_Before.before_slot_list[i]]))
